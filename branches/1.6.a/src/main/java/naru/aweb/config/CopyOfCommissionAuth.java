...
public class CopyOfCommissionAuth {
	private static JsonConfig jsonConfig;
	static{
		jsonConfig=new JsonConfig();
		jsonConfig.setRootClass(CopyOfCommissionAuth.class);
	...
	public static JSON collectionToJson(Collection<CopyOfCommissionAuth> commissionAuths){
		return JSONSerializer.toJSON(commissionAuths,jsonConfig);
	}
	
	private static final String COMMISSION_AUTH_QUERY="SELECT FROM " + CopyOfCommissionAuth.class.getName() + " ";
	/**
	 * @param jdoql
	 * @param firstResult
	 * @param maxResults
	 * @return
	 */
	public static Collection<CopyOfCommissionAuth> query(String whereSection,int from,int to,String ordering) {
		...
		return (Collection<CopyOfCommissionAuth>)pm.detachCopyAll((Collection<CopyOfCommissionAuth>)q.execute());
	}
	
	public static Collection<CopyOfCommissionAuth> getByKey(User user,String authUrl){
		PersistenceManager pm=JdoUtil.getPersistenceManager();
		Query query = pm.newQuery(CopyOfCommissionAuth.class, "user==:user && authUrl==:authUrl");
		Collection<CopyOfCommissionAuth> list=(Collection<CopyOfCommissionAuth>)query.execute(user,authUrl);
		return list;
	}
	public static Collection<CopyOfCommissionAuth> getByUser(User user){
		PersistenceManager pm=JdoUtil.getPersistenceManager();
		Query query = pm.newQuery(CopyOfCommissionAuth.class, "user==:user");
		Collection<CopyOfCommissionAuth> list=(Collection<CopyOfCommissionAuth>)query.execute(user);
		return list;
	}
	
	public static CopyOfCommissionAuth getById(Long id){
		PersistenceManager pm=JdoUtil.getPersistenceManager();
		return pm.getObjectById(CopyOfCommissionAuth.class, id);
	}
	...
	public CopyOfCommissionAuth(User user,String authUrl){
		this.user=user;
