...
public class Browser extends PoolBase implements Timer{
	...
	private boolean isAsyncStop=true;//asyncStop要求を受け付けたか否か?
	boolean isProcessing=false;//処理中か否か?
	...
	/**
	 * 単一リクエストのシミュレートをおこなう
	 * @param isHttps 接続先サーバssl種,requetHeader.isProxyがtrueの場合無視される(false相当)
	 * @param requestHeader
	...
	/**
	 * chIdが通知されるのは、trace1ショット実行の場合だけ。
	 * 1)callerが１つ。
	 * 2)traceが指定されている。
	 * この場合、非同期通知情報にAccessLogのidを通知する。
	 * 
	 * @param chId
	 */
...
//					System.out.println("dispatch+"+this.hashCode()+":size1:"+size1+":size2:"+size2+":caller:"+caller+":clientHandler:"+clientHandler);
			...
			if(clientHandler==null && caller!=null){
				//callerを取得しては見たがclientが得られなかったため、返却
				callerQueue.addLast(caller);
				caller=null;
			}
			if(processingClientHandler.size()==0){//もうイベントが発生する余地がない
				callerQueue.clear();
				isProcessing=false;
				isAsyncStop=true;
			}
