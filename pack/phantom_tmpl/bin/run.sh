...
${JAVA_HOME}/bin/java ${DEBUG_FLAG} -DPH_DEBUG="${PH_DEBUG}" -DPH_CONF_XML=${PH_CONF_XML} -DPH_VM_OPTIONS="${PH_VM_OPTIONS}" -DPH_HEAPSIZE=${PH_HEAPSIZE} -DPH_HOME=${PH_HOME} -DQUEUELET_HOME=${QUEUELET_HOME} -jar ${QUEUELET_BOOT_JAR} ${CONF_XML} $1 $2 $3 $4
